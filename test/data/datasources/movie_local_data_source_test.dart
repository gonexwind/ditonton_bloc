...
void main() {
  ...
  group('remove watchlist', () {
    test('should return success message when remove from database is success', () async {
      // arrange
      when(mockDatabaseHelper.removeWatchlist(id: testMovieTable.id)).thenAnswer((_) async => 1);
      // act
    ...
    test('should throw DatabaseException when remove from database is failed', () async {
      // arrange
      when(mockDatabaseHelper.removeWatchlist(id: testMovieTable.id)).thenThrow(Exception());
      // act
  ...
  group('Get Movie Detail By Id', () {
    ...
    test('should return Movie Detail Table when data is found', () async {
      // arrange
      when(mockDatabaseHelper.getDataById(id: tId)).thenAnswer((_) async => testMovieMap);
      // act
    ...
    test('should return null when data is not found', () async {
      // arrange
      when(mockDatabaseHelper.getDataById(id: tId)).thenAnswer((_) async => null);
      // act
      final result = await dataSource.getDataById(tId);
      // assert
      expect(result, null);
  ...
  group('get watchlist movies', () {
    test('should return list of MovieTable from database', () async {
      // arrange
      when(mockDatabaseHelper.getWatchlist()).thenAnswer((_) async => [testMovieMap]);
      // act
